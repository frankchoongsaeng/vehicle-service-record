...
async function ensureStripeCustomer(userId: string, email: string): Promise<string> {
    return withServerMonitoringSpan('billing.ensure_customer', { userId }, async () => {
...
router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        ...
        const subscription = await withServerMonitoringSpan(
            'billing.load_subscription_state',
            {
                requestId: req.requestId,
                userId: authUser.id
            },
            async () => getBillingSubscriptionState(authUser.id)
        )
...
router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        ...
        const session = await withServerMonitoringSpan(
        ...
        res.status(201).json({ url: session.session.url })
    })
)
...
router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
        const authUser = req.authUser!
        const user = await withServerMonitoringSpan(
            'billing.load_customer_portal_user',
            {
                requestId: req.requestId,
                userId: authUser.id
            },
            async () => getBillingUser(authUser.id)
        )
