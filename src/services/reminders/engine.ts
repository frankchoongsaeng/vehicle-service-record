...
export async function evaluateReminderNotifications(now = new Date()) {
    return withServerMonitoringSpan(
        'reminders.evaluate_notifications',
        {
        ...
        async () => {
            ...
            for (const user of users) {
                const queuedForUser = await withServerMonitoringSpan(
                    'reminders.evaluate_user',
                    {
                        userId: user.id,
                    ...
                    async () => {
                        ...
                        for (const vehicle of user.vehicles) {
                            ...
                            const effectiveRule =
                                vehicle.reminder_mode === 'custom' ? vehicle.reminder_rules[0] ?? null : workspaceRule
                        ...
                        await withServerMonitoringSpan(
                            'reminders.queue_digest',
                            {
                                userId: user.id,
                                itemCount: digest.itemCount,
                                dedupeKey,
                                existing: Boolean(existingNotification)
                            },
                            async () => {
                                if (existingNotification) {
                                    await prisma.notification.update({
                                        ...
                                        data: {
                                            ...
                                            status: existingNotification.sent_at
...
export async function processPendingReminderNotifications(now = new Date()) {
    return withServerMonitoringSpan(
        'reminders.process_pending_notifications',
        {
...
async function sendReminderNotification(
    ...
    await withServerMonitoringSpan(
        'reminders.deliver_notification',
        {
            notificationId: notification.id,
            userId: notification.user_id,
